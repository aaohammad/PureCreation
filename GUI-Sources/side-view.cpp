...
void SideGLWidget::drawInformation()
{
	glDisable (GL_LIGHTING);
	glColor3f(1.0, 1.0, 1.0);
	renderText(glWidgetWidth / 2 - 40, 20, "Side", fontOfOnScreenText);
...
void SideGLWidget::sceneTransformations()
{
	...
	glTranslatef(sceneTranslateXMoving, sceneTranslateYMoving, 0.0);
	glTranslatef(sceneTranslateX, sceneTranslateY, 0.0);
}

void SideGLWidget::initializeGL()
{
	glEnable(GL_DEPTH_TEST);
...
void SideGLWidget::paintGL()
{
	...
	if(selectionRectangle)
	{
		...
		glVertex3d(startDrawingPositionX, startDrawingPositionY, 0.0f);
		glVertex3d(finishDrawingPositionX, startDrawingPositionY, 0.0f);
		glVertex3d(finishDrawingPositionX, finishDrawingPositionY, 0.0f);
		glVertex3d(startDrawingPositionX, finishDrawingPositionY, 0.0f);
		glEnd();
...
void SideGLWidget::convertWindowCoorToSceneCoor(float &x, float &y, float &z, QPoint point_on_viewport)
{
	sceneTransformations();
	int viewport[4];
...
void SideGLWidget::pointSelection (int x, int y, QList<Model *> &selectedModels, QList<Vector *> &selectedVertices)
{
...
void SideGLWidget::rectangularSelection (int preX, int preY, int curX, int curY, QList<Model *> &selectedModels, QList<Vector *> &selectedVertices)
{
	GLuint selectBuffer[1024];
	glSelectBuffer(1024, selectBuffer);
	glRenderMode(GL_SELECT);
