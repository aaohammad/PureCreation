...
void TopGLWidget::drawInformation()
{
	glDisable (GL_LIGHTING);
	glColor3f(1.0, 1.0, 1.0);
	renderText(glWidgetWidth / 2 - 40, 20, "Top", fontOfOnScreenText);
	...
	glColor3f(1.0, 1.0, 1.0);
	renderText(-12, 110, 0, "-Z", fontOfOnScreenText);
	...
	glColor3f(1.0, 1.0, 1.0);
	renderText(110, -6, 0, "X", fontOfOnScreenText);
...
void TopGLWidget::sceneTransformations()
{
	...
	glTranslatef(sceneTranslateXMoving, 0.0, sceneTranslateZMoving);
	glTranslatef(sceneTranslateX, 0.0, sceneTranslateZ);
}

void TopGLWidget::initializeGL()
{
	glEnable(GL_DEPTH_TEST);
...
void TopGLWidget::paintGL()
{
	...
	if(selectionRectangle)
	{
		...
		glVertex3d(startDrawingPositionX, 0.0f, startDrawingPositionZ);
		glVertex3d(finishDrawingPositionX, 0.0f, startDrawingPositionZ);
		glVertex3d(finishDrawingPositionX, 0.0f, finishDrawingPositionZ);
		glVertex3d(startDrawingPositionX, 0.0f, finishDrawingPositionZ);
		glEnd();
...
void TopGLWidget::convertWindowCoorToSceneCoor(float &x, float &y, float &z, QPoint point_on_viewport)
{
	sceneTransformations();
	int viewport[4];
...
void TopGLWidget::pointSelection (int x, int y, QList<Model *> &selectedModels, QList<Vector *> &selectedVertices)
{
...
void TopGLWidget::rectangularSelection (int preX, int preY, int curX, int curY, QList<Model *> &selectedModels, QList<Vector *> &selectedVertices)
{
	GLuint selectBuffer[1024];
	glSelectBuffer(1024, selectBuffer);
	glRenderMode(GL_SELECT);
