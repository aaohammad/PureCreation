...
Bezier::Bezier(QList<Vector> vertices)
{
...
void Bezier::drawEdgeSelectMode()
{

}

void Bezier::drawFaceSelectMode()
{

}

void Bezier::drawObjectSelectMode()
{
    glPushMatrix();
    //between push matrix
    ...
    float ctrlPoints[vertexManager->size()][3];
    for(int i = 0; i < vertexManager->size() ; i++)
    {
        ctrlPoints[i][0] = vertexManager->at(i).x;
        ctrlPoints[i][1] = vertexManager->at(i).y;
        ctrlPoints[i][2] = 0.0;
    }
    glMap1f(GL_MAP1_VERTEX_3, 0.0f, 100.0f, 3, vertexManager->size(), &ctrlPoints[0][0]);
...
void Bezier::drawVertexSelectMode()
{

}

void Bezier::save(QTextStream *out)
{
    QString temp("Bezier");
    QString tab("\t");
    (*out) << temp;
...
void Bezier::reflect(float m, float n)
{
    int i = 0;
...
void Bezier::vertexComboBoxChanged()
{
    char f = 'f';
